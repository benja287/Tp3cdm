...
void formatearMensaje2(char* buffer, size_t buffer_size, uint8_t temperatura, uint8_t humedad) {
	// Variables para cadenas temporales
	char temp_str[4];
	char hum_str[4];
...
void estado_enviar_datos(void) {
	char buffer[77];
	if (get_time(buffer, sizeof(buffer))){
	}
...
//	uint8_t segundos, minutos, horas, dia, fecha, mes, ano;
	...
	//	DS3231_GetTime(&segundos, &minutos, &horas, &dia, &fecha, &mes, &ano);
      ...
      //  get_time(buffer, sizeof(buffer));
		...
		formatearMensaje2(buffer_datos, sizeof(buffer_datos), temperatura, humedad);
        strcat(buffer_datos,buffer);
